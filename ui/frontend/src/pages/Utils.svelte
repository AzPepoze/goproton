<script lang="ts">
  ...
  import { GetUtilsStatus, InstallLsfg, UninstallLsfg, GetSystemToolsStatus, DetectLosslessDll } from '../../wailsjs/go/main/App';
  ...
  let systemStatus: launcher.SystemToolsStatus & { hasLosslessDll: boolean } = { 
    hasGamescope: false, hasMangoHud: false, hasGameMode: false, hasLosslessDll: false 
  };
  ...
  onMount(() => {
    loadStatus();
    EventsOn("lsfg-install-progress", (msg: string) => {
      progressMessage = msg;
    });
  });
...
<div class="utils-container">
  ...
  <div class="section-container glass">
    ...
    {#if !systemStatus.hasGamescope || !systemStatus.hasMangoHud || !systemStatus.hasGameMode || !systemStatus.hasLosslessDll}
      <p class="warning-text">Some features may not work until you install these tools. LSFG-VK requires Lossless Scaling installed via Steam.</p>
    {/if}
  </div>
  ...
  <div class="utils-grid">
    ...
    <div class="util-card glass">
      ...
      <div class="description">
        <p>Lossless Scaling is a Windows-exclusive program featuring various algorithms for scaling and interpolating programs.</p>
        <p><strong>lsfg-vk</strong> is a Vulkan layer that hooks into Vulkan applications and generates additional frames using Lossless Scaling's frame generation algorithm.</p>
      ...
      <div class="action-area">
        ...
        {:else}
          <div class="install-controls">
            ...
            {#if isInstalling && progressMessage}
    ...
    <!-- Future utils can go here -->
    <div class="util-card glass placeholder">
      <div class="util-header">
        <div class="icon-bg">🛠️</div>
        <h3>More Utilities Coming Soon</h3>
      </div>
      <p class="description">
        We are working on integrating more tools like vkBasalt, MangoHud managers, and more.
      </p>
    </div>
  </div>
</div>
...
<style lang="scss">
  ...
  .section-container {
    ...
    h3 { margin: 0 0 16px 0; font-size: 1rem; color: var(--text-dim); text-transform: uppercase; letter-spacing: 1px; }
  }
  ...
  .status-item {
    ...
    .value { font-size: 0.8rem; opacity: 0.8; }

    &.ok {
      .dot { background: #10b981; box-shadow: 0 0 8px #10b981; }
    }
  }

  .warning-text { font-size: 0.8rem; color: var(--accent-secondary); margin: 12px 0 0 0; font-style: italic; }
