...
func BuildCommand(opts LaunchOptions) ([]string, []string) {
	...
	if opts.EnableLsfgVk {
		...
		// Support both v1.x and v2.x layer names
		lsfgLayers := "VK_LAYER_LSFGVK_frame_generation:VK_LAYER_LS_frame_generation"
		lsfgEnv = append(lsfgEnv, "VK_LOADER_LAYERS_ENABLE="+lsfgLayers)
		lsfgEnv = append(lsfgEnv, "VK_INSTANCE_LAYERS="+lsfgLayers)
		...
		// Important: Mount tools directory into the container
		lsfgEnv = append(lsfgEnv, "PRESSURE_VESSEL_FILESYSTEMS_RW="+lsfgDir)

		if opts.LsfgMultiplier != "" {
			lsfgEnv = append(lsfgEnv, "LSFG_MULTIPLIER="+opts.LsfgMultiplier)
		}
		if opts.LsfgPerfMode {
			lsfgEnv = append(lsfgEnv, "LSFG_PERFORMANCE_MODE=1")
		}
		...
		if dllPath != "" {
			lsfgEnv = append(lsfgEnv, "LSFG_DLL_PATH="+dllPath)
		}
	}
