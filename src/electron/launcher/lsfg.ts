...
import os from 'os';
import { expandPath, getBaseDir } from './utils.js';
import { LsfgConfigFile, LsfgConfigProfile } from './types.js';
...
function parseLsfgToml(data: string): LsfgConfigFile {
    ...
    for (let line of lines) {
        ...
        if (val === 'true') parsedVal = true;
            ...
            parsedVal = val.slice(1, -1).split(',').map(s => s.trim().replace(/^"|"$/g, '')).filter(s => s);
        }
...
export function saveLsfgProfile(gamePath: string, opts: Partial<LsfgConfigProfile & { allowFp16: boolean, dllPath: string }>): void {
    const configPath = getLsfgConfigPath();
